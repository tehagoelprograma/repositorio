...
public class Main {
	...
	private static void writePDF() {
		...
		private static void conectarConBBDD() {
			...
			String[] ordenMatriculas = new String[800];
				...
				for (int a = 0 ; a < numFotos ; a++) {
					...
					if (imagenExiste.exists()) {
						...
						try {
							...
							System.out.println("error en:" + path + FOTOS_REDIMENSIONADAS + listadoMatriculas[a] + "_recorte.jpg");
							JOptionPane.showMessageDialog(null, "Ha habido un problema con la foto: " + path + FOTOS_REDIMENSIONADAS + listadoMatriculas[a] + "_recorte.jpg"+ "; por favor, revisa esta foto.");
						}
    ...
    Este método se utiliza para almacenar la imagen en disco
		...
		private static void recortar (BufferedImage bufferedImage, String pathName) {
			...
			}catch (Exception e) { System.out.println(" la foto que da problemas es : " + pathName);
			 JOptionPane.showMessageDialog(null, "Ha habido un problema al recortar la foto: " + pathName+ "; por favor, revisa esta foto.");
			}
		} 
